...
#define imageOffset 0
	...
	for (i = 0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
		...
		if (size > 0) {
			...
			BOOL _result = VirtualProtect(reinterpret_cast<LPVOID>(static_cast<POINTER_TYPE>(section->Misc.PhysicalAddress | imageOffset)), size, protect, &oldProtect);
...
static void ExecuteTLS(PMEMORYMODULE module)
{
	...
	if (directory->VirtualAddress > 0) {
		PIMAGE_TLS_DIRECTORY tls = reinterpret_cast<PIMAGE_TLS_DIRECTORY>((codeBase + directory->VirtualAddress));
...
static int PerformBaseRelocation(PMEMORYMODULE module, SIZE_T delta)
{
	...
	if (directory->Size > 0) {
		PIMAGE_BASE_RELOCATION relocation = reinterpret_cast<PIMAGE_BASE_RELOCATION>(codeBase + directory->VirtualAddress);
		for (; relocation->VirtualAddress > 0;) {
			...
			relocation = reinterpret_cast<PIMAGE_BASE_RELOCATION>((((char *)relocation) + relocation->SizeOfBlock));
		}
		result = 1;
	}
	return result;
...
static int BuildImportTable(PMEMORYMODULE module)
{
	...
	if (directory->Size > 0) {
		...
		for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++)
		{
			...
			FARPROC *funcRef;
			HCUSTOMMODULE handle = module->loadLibrary(reinterpret_cast<LPCSTR>(codeBase + importDesc->Name), module->userdata);
			if (handle == NULL) {
			...
			for (; *thunkRef; thunkRef++, funcRef++) {
				if (IMAGE_SNAP_BY_ORDINAL(*thunkRef)) {
					*funcRef = module->getProcAddress(handle, reinterpret_cast<LPCSTR>(IMAGE_ORDINAL(*thunkRef)), module->userdata);
				}
				else {
					PIMAGE_IMPORT_BY_NAME thunkData = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(codeBase + (*thunkRef));
					*funcRef = module->getProcAddress(handle, reinterpret_cast<LPCSTR>(&thunkData->Name), module->userdata);
				}
				if (*funcRef == 0) {
					result = 0;
					break;
